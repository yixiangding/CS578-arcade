...
public class BatchDepFinder {
	...
	static String DepExt = "ext-tools/DependencyFinder";
	
//	static String DepExt = "C:\\DependencyFinder\\bin";
...
//	static String[] args	= new String[]{testInput, testOutput, testClass};
	...
	public static void single(final File versionFolder, final File outputDir, final String classesDirName, final String language) throws IOException {
		...
		// Run deps for a single folder for windows
		String cmd = "cmd /c DependencyExtractor.bat -xml -out "+ depsXMLFilename + " " + absoluteClassesDir;
		Process p =  Runtime.getRuntime().exec(cmd, null, new File(DepExt));
//		System.out.println(p.getOutputStream());
		// Run deps for a single folder for Linux
//		Process p =  Runtime.getRuntime().exec("export DependencyFinder_Home="+ DepExt);
		
//		Process p =  Runtime.getRuntime().exec("java -classpath %DEPENDENCYFINDER_HOME%/classes;%DEPENDENCYFINDER_HOME%/lib/DependencyFinder.jar;%DEPENDENCYFINDER_HOME%/lib/guava.jar;%DEPENDENCYFINDER_HOME%/lib/jakarta-oro.jar;%DEPENDENCYFINDER_HOME%/lib/jsr305.jar;%DEPENDENCYFINDER_HOME%/lib/log4j.jar;%CLASSPATH% com.jeantessier.dependencyfinder.cli.DependencyExtractor -xml -out "+ depsXMLFilename + " "
